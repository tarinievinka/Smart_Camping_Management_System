...
const GuideAllReviews = () => {
  const navigate = useNavigate();
  const [reviews, setReviews] = useState([]);
  const [loading, setLoading] = useState(true);
  const [typeFilter, setTypeFilter] = useState("guide");
      ...
      <div className="flex-1 h-screen overflow-y-auto">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">Guide Reviews</h1>
              <p className="text-slate-500 font-medium">All feedback received for system guides</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center shadow-sm max-w-2xl mx-auto my-10">
              ...
              <p className="text-slate-500 text-base max-w-sm mx-auto mb-8">
                There are currently no reviews for guides.
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-green-50 border border-green-100 text-green-700 text-xs font-semibold">
                            <MapPin size={12} />
                            {review.targetType}
                          </span>
                        </div>
...
export default GuideAllReviews;
